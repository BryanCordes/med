...
const handlers = {
  ...
  'session-restore': async () => {
    ...
    // Sync ai-dlc knowledge files into ruflo memory (background, non-blocking)
    {
      const { spawn } = require('child_process');
      const projectRoot = path.resolve(helpersDir, '../..');
      const syncScript = path.join(projectRoot, 'scripts', 'ai-dlc-ruflo-sync.cjs');
      if (fs.existsSync(syncScript)) {
        spawn('node', [syncScript], {
          detached: true,
          stdio: 'ignore',
          cwd: projectRoot,
        }).unref();
        console.log('[AI-DLC] Syncing knowledge to ruflo memory (background)');
      }
    }
  },
